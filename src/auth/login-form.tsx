...
export function LoginForm({
  ...
  }, [phase, state?.fromRegister]) // 当阶段改变时重新计算高度
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    if (email && !isLoading) {
      ...
      setIsLoading(false)
      handlePhaseChange("password", false) // 前进
    }
  }
  ...
  const handleBack = () => {
    if (phase === "password") {
      handlePhaseChange("email", true) // 返回到邮箱输入
    } else if (phase === "email") {
      handlePhaseChange("initial", true) // 返回到初始界面
    }
  }
    ...
    <div className={cn("flex flex-col gap-3", className)} {...props}>
      <Card>
        <div className="flex flex-row items-center justify-between pr-5">
          {/* 恢复 v2 的 Logo 布局结构，但使用 Cloudrave Logo 组件 */}
