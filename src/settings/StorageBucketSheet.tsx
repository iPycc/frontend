            ...
            <div className="space-y-5">
              ...
              >
                <CheckboxRow
                  ...
                  onToggle={() =>
                    updateDraft("corsConfigured", !draft.corsConfigured)
                  }
                />
              </FieldBlock>
            </div>
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                <FieldBlock label="分片阈值" hint="超过该值开始使用 multipart 上传。">
                  <Input
                    ...
                    onChange={(event) =>
                      updateDraft("multipartThreshold", event.target.value)
                    }
                  />
                </FieldBlock>
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                <FieldBlock label="并发数" hint="控制分片并发上传的数量。">
                  <Input
                    ...
                    onChange={(event) =>
                      updateDraft("concurrency", Number(event.target.value || 1))
                    }
                  />
                </FieldBlock>
              </div>
                ...
                >
                  ...
                  >
                    <SelectField
                      ...
                      onChange={(value) =>
                        updateDraft("protocol", value as "https" | "http")
                      }
                      options={[
          ...
          {step === 5 ? (
            <div className="space-y-4">
              ...
              <dl className="grid gap-3 text-sm sm:grid-cols-2">
                ...
                <BucketMeta
                  label="CORS"
                  value={draft.corsConfigured ? "已确认" : "待确认"}
                />
                <BucketMeta
                  label="高级模式"
                  value={draft.advancedMode ? "已开启" : "已关闭"}
                />
                <BucketMeta
                  label="上传策略"
                  value={`${draft.multipartThreshold} / ${draft.partSize}`}
                />
                <BucketMeta
                  label="签名与并发"
                  value={`${draft.presignTtl}s / ${draft.concurrency} 并发`}
                />
              </dl>
            </div>
          ) : null}
